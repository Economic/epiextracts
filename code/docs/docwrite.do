...
* if an R snippet exists for this variable (generated by
* generate_r_snippets.R from the r_pilot/ port), show that INSTEAD of the
* Stata code -- otherwise fall back to the Stata generate_`1'.do exactly as
* before. `codefence' controls which language the ## Code block is fenced as.
tempfile varcode
local tripletick = "```"
local codefence stata
capture confirm file ${codedocs}r_snippets/`1'_rcode.R
if _rc == 0 {
	copy ${codedocs}r_snippets/`1'_rcode.R `varcode'
	local codefence r
}
else {
...
webdoc put     `tripletick'`codefence'
webdoc append `varcode'
webdoc put     `tripletick'
webdoc put
