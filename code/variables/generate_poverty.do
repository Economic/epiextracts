...
if $marchcps == 1 { 
    ...
    if tm(1976m1) <= $date & $date <= tm(1979m12) {
        gen byte universe = 1 if inlist(famtyp, 0, 4, 3) & 14 <= age
        replace poverty = 0 if 2 <= povlev & universe == 1         
        replace poverty = 1 if povlev == 1 & universe == 1
        replace poverty = . if universe != 1
    }
    if tm(1980m1) <= $date & $date <= tm(1987m12) {
        gen byte universe = 1 if inlist(famtyp, 0, 4, 3) & 15 <= age
        replace poverty = 1 if povlev == 1 & universe == 1
        replace poverty = 0 if 2 <= povlev & universe == 1
        replace poverty = . if universe != 1
    }
    if tm(1988m1) <= $date & $date <= tm(1997m12) {
        gen byte universe = 1 if 1 <= famtyp & famtyp <= 3 & 15 <= age
        replace poverty = 1 if povlev == 1 & universe == 1
        replace poverty = 0 if 2 <= povlev & universe == 1
        replace poverty = . if universe != 1
    }
    if tm(1998m1) <= $date {
        gen byte universe = 1 if 1 <= a_famtyp & a_famtyp <= 3 & 15 <= age
...
notes poverty: Universe definition from 1976-1979: all persons except for unrelated individuals under age 14
notes poverty: Universe definition from 1968-1979: all persons except for unrelated individuals under age 15
notes poverty: 1976-1997 Unicon: povlev
notes poverty: 1998-present CPS: famlis