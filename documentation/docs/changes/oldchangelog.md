...
See the newer [changelog](changelog.md) for more recent changes.
...
* Trimming of extreme wage values
	`wage` and wage`otc set to missing if under 50 cents/hour or over $100/hour in 1989$.
...
* `basicwgt` is now final weight through 1997, composite weight `cmpgwgt` 1998-present
